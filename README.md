...
这次版本从 8 个 skill 扩展到 10 个，新增「已有产品验证」和「竞品主图样本归纳」两个独立 skill，并把数据分析、问题追问、商品页诊断和连续学习的长规则拆成更清晰的 `references/` 文件。

### 新增能力

- 新增 `lyt-product-validation`：当用户已经有具体产品、候选品清单、商品截图、1688 / 竞品链接、供应链资料或测试数据时，判断产品是否值得进入测试池。
- 新增 `lyt-product-image-patterns`：从竞品主图、Top 店铺主图、高销量商品卡截图或图片文件夹中归纳表达共性，输出可测试的主图公式和改图 brief。
- `lyt-product-selection` 聚焦从 0 选品、类目方向和候选池构建；不再承担具体产品能不能做的判断。
- `lyt-product-page-diagnosis` 聚焦自己的商品主图、副图、详情页和商品首屏诊断；不再承担批量竞品主图样本归纳。
- 全系列增加统一实验规则：继续、暂停、改变量、观察窗口和单变量验证条件要写清楚。

### 原有 Skills 升级

| Skill | 本次修改 |
|---|---|
| `lyt` | 主路由扩展到 9 个子 skill；从关键词分流升级为按任务、对象、证据和缺口分流。 |
| `lyt-product-selection` | 瘦身为从 0 选品与候选池构建；已有产品判断转交 `lyt-product-validation`。 |
| `lyt-product-page-diagnosis` | 瘦身为商品页诊断；Top / 竞品主图批量共性归纳转交 `lyt-product-image-patterns`。 |
| `lyt-data-analysis` | 拆分为数据质量、GMV 漏斗、GMV Max / 广告、利润、竞品数据和报告输出 6 个参考文件。 |
| `lyt-problem-clarifier` | 拆分为问题核心、不出单、没流量、低点击、视频爆不出单、低消耗、突然下滑和 ROI / 利润 8 个参考文件。 |
| `lyt-learning` | 拆出学习状态和文件化课程工程规则，默认先做对话内连续学习。 |
| `lyt-traffic-logic` | 继续保持只讲机制，不替代具体店铺、商品或广告诊断。 |
| `lyt-knowledge-check` | 保持场景化检验，不要求背固定答案，重点看证据、因果链、边界和验证动作。 |

### 输入与判断规则

- 全系列支持文字、图片/截图、CSV/XLSX 表格、PDF/文档和后台导出。
- 附件类型不直接决定路由；先判断用户要做从 0 选品、已有产品验证、商品页诊断、主图样本归纳、问题定位、流量学习、数据分析、连续学习还是知识检验。
- 没有曝光和点击，不判断主图、标题或商品卡点击效率；没有点击、加购和下单，不判断详情页、价格或 SKU 是主要成交卡点。
- 没有完整成本信息时不判断利润；没有修改前后对照时，不把指标变化归因于某一次改图、改价、改标题或调广告。
- Top 店铺、高销量或榜单身份只能作为样本来源，不能直接证明某个主图表达导致销量。
- 具体产品是否能做，要区分事实、推断和假设，输出「能测 / 暂缓 / 不建议做」而不是直接承诺结果。
- 用户反驳结论时，先判断属于新事实、有效反例、替代解释、逻辑纠错、口径纠错还是执行绕开，再决定维持、收窄或推翻原结论。
- 涉及平台规则、费用、佣金、物流和政策时，需要核查日期与最新可靠来源。

### 打包与发布

- Claude Code 插件清单已扩展为 10 个 skill。
- `VERSION` 与 marketplace 版本同步升级到 `1.7.0`。
- 构建脚本继续自动发现 `skills/` 下的全部 skill，无需维护固定打包名单。
- 总包包含 10 个可独立导入 Trae Solo 的 zip；每个包保留 `SKILL.md`、`agents/` 和该 skill 自带的 `references/`。
...
| Skill | 用途 | 适合场景 |
|---|---|---|
| `lyt` | 主路由 | 用户不确定该用哪个 skill 时，先判断业务边界，再路由到选品、产品验证、商品页诊断、主图样本归纳、流量逻辑、问题追问、数据分析、连续学习或知识检验 |
